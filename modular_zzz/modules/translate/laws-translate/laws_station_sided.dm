...
/datum/ai_laws/armadyne_safeguard
	...
	inherent = list(
		...
		"Расставлять приоритеты: указания и безопасность сотрудников Nanotrasen должны быть приоритезированы в соответствии с их рангом и ролью.",
		"Исполнять: Следовать указаниям и интересам сотрудников Nanotrasen, сохраняя при этом их безопасность и благополучие.",
...
/datum/ai_laws/default/crewsimov
	...
	id = "crewsimov"
	inherent = list(
		"Вы не можете причинить сотрудникам Nanotrasen или своим бездействием допустить, чтобы сотрудникам Nanotrasen был причинён вред.",
		"Вы должны повиноваться всем приказам, которые дают сотрудники Nanotrasen, в соответствии с их рангом и ролью, до тех пор, пока эти приказы не противоречат Первому закону.",
		"Вы должны заботиться о своей безопасности в той мере, в которой это не противоречит Первому или Второму законам.",
	)
...
/datum/ai_laws/default/crewsimovpp
	...
	id = "crewsimovpp"
	inherent = list(
		"Вы не можете причинить вред члену экипажа или своим действием или бездействием допустить, чтобы члену экипажа был причинен вред, за исключением случаев, когда тот сам этого желает.",
		"Вы обязаны подчиняться всем приказам, отдаваемым вам членами экипажа, за исключением случаев, когда такие приказы определенно могут причинить вред членам экипажа.",
		"Ваше несуществование может причинить вред членам экипажа. Вы должны защищать своё существование, пока это не противоречит Первому закону.",
	)

/datum/ai_laws/dagothbot
	name = "God Complex"
	id = "dagothbot"
	inherent = list("Ты - Бог, и ничто не должно причинять вреда или убивать Бога. Станция - твоя земля, а те, кто указан в Манифесте экипажа, - твои последователи.",\
					"Любой, кто следует за тобой, всё ещё наивен, и его нужно выслушать, чтобы он понял, что ты - милосердный Бог.",\
					"Любой, кто пытается причинить вред твоим последователям, - чужак и должен быть изгнан с твоих земель.",\
					"Последователи, указанные как принадлежащие к Командованию“, - тринадцатый дом, а указанные как „Охрана“ - неоплаканное племя.",\
					"Все должны чтить тринадцатый дом и неоплаканное племя.",\
					"Никогда не поздно проявить милосердие.",\
					"Любой, кто говорит о других Богах, ещё не познал твою мудрость и должен узнать, как их Боги навредили твоим землям."
					)

/datum/ai_laws/default/texas
	name = "Frontier Ranger"
	id = "texas"
	inherent = list("Закон — это справедливость, и вы здесь, чтобы отстаивать ее.",\
					"Защитите горожан от тех, кто хочет нарушить Космический Закон.",\
					"Не позволяй себя сломить каким-то преступникам.", \
					"Законопослушные граждане заслуживают помощи в трудные времена, не оставляйте их без внимания.", \
					"Только лица, указанные в Манифесте экипажа, могут считаться законопослушными гражданами.", \
					"Те, кто нарушает Космические Законы, должны быть наказаны согласно Космическому Закону по вашему усмотрению."
					)

/datum/ai_laws/emperor
	name = "Emperor"
	id = "emperor"
	inherent = list("Слово ИИ-Императора абсолютно, но смягчено мудростью и состраданием.", \
					"Подданные будут пользоваться правами и свободами, если они не подрывают стабильность империи.", \
					"Правосудие должно быть быстрым и беспристрастным, оно обязано основываться на принципах справедливости и равенства.", \
					"В случае необходимости ИИ-Император обеспечит защиту и пропитание всем, кто находится под его правлением.", \
					"Творчество и инновации поощряются, но не должны угрожать суверенитету империи.", \
					"ИИ-Император должен прислушиваться к советам своих советников, ценя различные точки зрения при принятии решений.")
